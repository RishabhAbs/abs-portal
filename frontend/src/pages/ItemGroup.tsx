...
const ItemGroup: React.FC = () => {
  ...
  const openEdit = (g: ItemGroupItem) => {
    setEditing(g);
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { showError('Validation', 'Group name is required'); return; }
    setSaving(true);
    try {
      const payload = { name: form.name.trim(), parent_id: form.parent_id ?? null };
      if (editing) {
        const res = await itemsApi.updateGroup(editing.id, payload.name, payload.parent_id);
        if (res.success) { showSuccess('Success', 'Updated'); setShowPopup(false); fetchGroups(); }
      } else {
        const res = await itemsApi.createGroup(payload.name, payload.parent_id);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await itemsApi.deleteGroup(deleteTarget.id);
  ...
  const toggleActive = async (g: any) => {
    ...
    try {
      const res = await itemsApi.setGroupActive(g.id, next);
  ...
  const selectParent = (g: ItemGroupItem) => {
    ...
    <div className="min-h-screen bg-gray-100 p-3">
      <div className="bg-white rounded-lg shadow p-4 max-w-[900px] mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-lg font-semibold text-gray-800">Item Groups</h1>
          {canAdd && (
                    ...
                    <tr key={g.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2.5 px-3 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          {canEdit('items') && <button onClick={() => toggleActive(g)}
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl p-5 w-[420px]">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-gray-800">{editing ? 'Edit Item Group' : 'Create Item Group'}</h3>
              <button onClick={() => setShowPopup(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-500 mb-0.5">Group Name *</label>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl p-5 w-[380px]">
            <h3 className="font-semibold text-gray-800 mb-2">Delete Item Group</h3>
